...
int PlayGame(char Matrix[max][max], char NamePlayer1, char NamePlayer2, string check)
{
	int player,count;
	...
	do
	{
		switch(player)
		{
			case 1: 
				...
				if (checkEmpty(Matrix,row,column) == true) Matrix[row][column] = 'X';
				else
				{
					do
					{
			...
			case 2:
				...
				if (checkEmpty(Matrix,row,column)) Matrix[row][column] = 'O';
				else
				{
					do
					{
...
int main()
{
	char Matrix[max][max] = {' ',' ',' ',' ',' ',' ',' ',' ',' '};
	int choice;
	char NamePlayer1, NamePlayer2,a;
	string kt="playing";
	...
	do{
		...
		{
			case 1:
				cout<<"Enter your name Player 1: "; cout<<NamePlayer1;
				cout<<"\nEntern your name Player 2: "; cout<<NamePlayer2;
				Board(Matrix);
